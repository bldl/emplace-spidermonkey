...
bool MapObject::getOrInsertComputed(JSContext* cx, HandleValue key,
  ...
  if (const Table::Entry* p = Table(this).get(k)) {
    ...
    if (needsPostBarriers) {
      ...
      if (!Table(this).put(cx, k, value)) {
        return false;
      }
    } else {
      // Use the PreBarrieredTable representation which does not.
      if (!PreBarrieredTable(this).put(cx, k, value)) {
        return false;
      }
    }
    rval.set(value);
...
bool MapObject::getOrInsertComputed_impl(JSContext* cx, const CallArgs& args) {
  auto* mapObj = &args.thisv().toObject().as<MapObject>();
  return mapObj->getOrInsertComputed(cx, args.get(0), args.get(1), args.rval());
}
